...
SELECT
  ...
  , CAST(subq_14.booking_value_with_is_instant_constraint AS DOUBLE) / CAST(NULLIF(subq_14.booking_value, 0) AS DOUBLE) AS instant_booking_value_ratio
FROM (
  ...
  SELECT
    ...
    , MAX(subq_13.booking_value) AS booking_value
  FROM (
    -- Compute Metrics via Expressions
    SELECT
    ...
    FROM (
      ...
      FROM (
        ...
        FROM (
          ...
          WHERE subq_9.metric_time__day BETWEEN '2000-01-01' AND '2040-12-31'
        ) subq_10
      ) subq_11
      GROUP BY
        subq_11.metric_time__day
