...
class WhereFilterTimeDimensionFactory(ProtocolHint[QueryInterfaceTimeDimensionFactory]):
    ...
    def create(
        ...
        return WhereFilterTimeDimension(
            ...
            entity_links=tuple(EntityReference(entity_link_name) for entity_link_name in entity_path)
            + structured_name.entity_links,
            time_grain=TimeGranularity(time_granularity_name) if time_granularity_name else None,
            date_part=DatePart(date_part_name) if date_part_name else None,
        )
