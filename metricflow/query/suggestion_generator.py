...
class QueryItemSuggestionGenerator:
    ...
    def __init__(  # noqa: D
        self, input_naming_scheme: QueryItemNamingScheme, input_obj_str: str, candidate_filter: Optional[SpecPattern]
    ) -> None:
        self._input_naming_scheme = input_naming_scheme
        self._input_obj_str = input_obj_str
    ...
    def suggested_specs(
        ...
        # Using only the base grain could give a better variety of suggestions (instead of one at various grains).
        # This might miss the correct grain, so more thought is needed to produce the best set of suggestions.
        candidate_specs = BaseTimeGrainPattern().match(candidate_specs)
        ...
        return tuple(spec_str_to_spec[similarity_str] for similarity_str in top_similarity_strs)
