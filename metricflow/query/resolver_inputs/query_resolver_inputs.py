...
@dataclass(frozen=True)
class ResolverInputForDefinedWhereFilterIntersection(MetricFlowQueryResolverInput):
    """An input to describe a where filter that's defined in the manifest.

    Treating the defined filter as a resolver input so that it can go through the same error flows as the other query
    inputs.
    """
