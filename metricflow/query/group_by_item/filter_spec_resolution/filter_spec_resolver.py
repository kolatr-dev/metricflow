...
class _ResolveWhereFilterSpecVisitor(GroupByItemResolutionNodeVisitor[FilterSpecResolutionLookUp]):
    ...
    def _resolve_specs_for_where_filters(
        ...
        for call_parameter_set, spec_pattern in call_parameter_set_to_pattern.items():
            ...
            # The paths in the issue set were generated relative to the current node. To give the paths relative to the
            # resolution_path, we have to add the nodes. The last one is excluded since those will be the same. e.g.
            # A->B and B->C should result in A->B->C.
            issue_set_with_resolution_path_prefix = group_by_item_resolution.issue_set.with_path_prefix(
                MetricFlowQueryResolutionPath(resolution_path_nodes=resolution_path.resolution_path_nodes[:-1])
            )
            ...
            resolutions.append(
                FilterSpecResolution(
                    ...
                    where_filter_intersection=where_filter_intersection,
                    spec_pattern=spec_pattern,
                    issue_set=issue_set_with_resolution_path_prefix,
                )
            )
