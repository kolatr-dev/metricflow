...
from metricflow.query.issues.parsing.invalid_where import WhereFilterParsingIssue
...
class FilterSpecResolutionLookUp(Mergeable, PathPrefixable):
    """Allows you to look up the spec for a call parameter set in a where filter."""
...
class _ResolveWhereFilterSpecVisitor(GroupByItemResolutionNodeVisitor[FilterSpecResolutionLookUp]):
    """Visitor that resolves specs for filters and builds a lookup.

    This visitor traverses the group-by-item resolution DAG and if there is a where filter associated with the node,
    this resolves (potentially) ambiguous group-by-items that are specified in the filter. All resolutions are
