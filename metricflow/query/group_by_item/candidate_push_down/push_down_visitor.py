        ...
        e.g.
            ...
            def visit_some_node(self, node) -> Result:
                with track_node_visit(node) as current_traversal_path:
                    # current_traversal_path contain the nodes visited from the start.
        """
...
class _PushDownGroupByItemCandidatesVisitor(GroupByItemResolutionNodeVisitor[PushDownResult]):
    ...
    ambiguous group-by-item pattern from the leaf nodes (representing measures / the specs available for those
    ...
    def visit_measure_node(self, node: MeasureGroupByItemResolutionNode) -> PushDownResult:
        """At the measure node, push group-by-item specs that match the patterns to the child node."""
            ...
            if len(matching_specs) == 0:
                return PushDownResult(
                    ...
                    issue_set=MetricFlowQueryResolutionIssueSet.from_issue(
                        NoMatchingGroupByItemsAtRoot.from_parameters(
                            parent_issues=(),
    ...
    def visit_query_node(self, node: QueryGroupByItemResolutionNode) -> PushDownResult:
        """Similar to the metric node - intersect candidates from the parents and pass them to the children."""
        with self._path_from_start_node_tracker.track_node_visit(node) as current_traversal_path:
            merged_result_from_parents = self._merge_push_down_results_from_parents(
                push_down_results_from_parents={
                    parent_node: parent_node.accept(self) for parent_node in node.parent_nodes
    ...
    @override
    def visit_no_metrics_query_node(self, node: NoMetricsQueryGroupByItemResolutionNode) -> PushDownResult:
