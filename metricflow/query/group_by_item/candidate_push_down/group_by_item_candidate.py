...
class GroupByItemCandidateSet(PathPrefixable):
    ...
    # The path from the leaf node to the measure node where the specs originated. There can be multiple paths as
    # when candidate sets are merged, there can be multiple measures were a spec could have come from.
    measure_paths: Tuple[MetricFlowQueryResolutionPath, ...]
    # The path from the leaf node to the node where this set was generated.
